...
- **Suggestions**: A list of suggestions that comes from the API firstly, and then after it comes from the local database, it is random and it's always 8 of them. If the last search term is equal to one of the suggestions, it's going to be in a different color. Any of them can be tapped to be used as a search term.
...
The project uses Github Actions to run the [CI scripts](./.github/workflows/android-pr.yml) when a Pull Request is opened or a Tag is created.


### Pull Request
- **Static code analysis**: The script uses the *detekt* gradle plugin to check if all the code is formatted consistently and there are not **any** issues with complexity.
...
---
### Tag
- **APK build and upload**: The script generates a debug apk and uploads it as a Github Action artifact.
